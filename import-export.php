...
function booking_export( $args = array() ) {
	...
	$pluginCalendars = $wpdb->get_results( "SELECT * FROM {$wpdb->prefix}hotel_booking_calendars" );
	$pluginAvailabilities = $wpdb->get_results( "SELECT * FROM {$wpdb->prefix}hotel_booking_availability" );
	$pluginReservations = $wpdb->get_results( "SELECT * FROM {$wpdb->prefix}hotel_booking_reservation" );
	$pluginSettings = $wpdb->get_results( "SELECT * FROM {$wpdb->prefix}hotel_booking_settings" );
...
function hotel_booking_export_func() {
	global $wpdb;
	if ( !current_user_can( 'manage_options' ) ) {
      wp_die( 'You are not allowed to be on this page.' );
	}
	check_admin_referer( 'hotel_booking_export_verify' );
		
	/* Trigger download of .xml export file */
	booking_export();
		
}
add_action( 'admin_post_hotel_booking_export', 'hotel_booking_export_func' );
	
	
	
function hotel_booking_import_func() {
	global $wpdb;
	if ( !current_user_can( 'manage_options' ) ) {
      wp_die( __('You are not allowed to be on this page.','hotel') );
	}
	check_admin_referer( 'hotel_booking_import_verify' );
 
	/* Perform checks on file: */
	// Sanity check
	if ( empty( $_FILES["booking_import"] ) )
		wp_die( __('No file found','hotel') );
 		
	$file = $_FILES["booking_import"];
		
	// Is it of the expected type?
	if ( $file["type"] != "text/xml" )
		wp_die( sprintf( __( "There was an error importing the logs. File type detected: '%s'. 'text/xml' expected", 'hotel'), $file['type'] ) );
 
	// Impose a limit on the size of the uploaded file. Max 2097152 bytes = 2MB
	if ( $file["size"] > 2097152 ) {
		$size = size_format( $file['size'], 2 );
		wp_die( sprintf( __( 'File size too large (%s). Maximum 2MB', 'import-logs', 'hotel' ), $size ) );
	}
 
	if( $file["error"] > 0 )
		wp_die( sprintf( __( "Error encountered: %d" , 'hotel' ), $file["error"] ) );
...
add_action( 'admin_post_hotel_booking_import', 'hotel_booking_import_func' );
...
function import( $file ) {
	...
	if ( isset( $booking["calendars"] ) ) {
		// //Clear Current Table
		$wpdb->query( "TRUNCATE TABLE ".$wpdb->prefix."hotel_booking_calendars" );
		...
		for ( $i=0;$i<count($booking["calendars"]);$i++ ){
			$calendarGood = $wpdb->insert(
				$wpdb->prefix."hotel_booking_calendars",
				array(
	...
	if ( isset( $booking["availability"] ) ) {
		// //Clear Current Table
		$wpdb->query( "TRUNCATE TABLE ".$wpdb->prefix."hotel_booking_availability" );
			
		// Load extracted from XML data to coresponding table in database
		for ( $i=0;$i<count($booking["availability"]);$i++ ){
			$availabilityGood = $wpdb->insert(
				$wpdb->prefix."hotel_booking_availability",
				array(
	...
	if ( isset( $booking["reservation"] ) ) {
		// //Clear Current Table
		$wpdb->query( "TRUNCATE TABLE ".$wpdb->prefix."hotel_booking_reservation" );
		
		// Load extracted from XML data to coresponding table in database
		for ( $i=0;$i<count($booking["reservation"]);$i++ ){
			$reservationGood = $wpdb->insert(
				$wpdb->prefix."hotel_booking_reservation",
				array(
	...
	if ( isset( $booking["settings"] ) ) {
		// //Clear Current Table
		$wpdb->query( "TRUNCATE TABLE ".$wpdb->prefix."hotel_booking_settings" );
			
		// Load extracted from XML data to coresponding table in database
		for ( $i=0;$i<count($booking["settings"]);$i++ ){
			$settingsGood = $wpdb->insert(
				$wpdb->prefix."hotel_booking_settings",
				array(
				...
				array ( "%d", "%s", "%s", "%s", "%d", "%d", "%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", "%d", "%s", "%s", "%s", "%s", "%d" )
			);
		}	 
	}
